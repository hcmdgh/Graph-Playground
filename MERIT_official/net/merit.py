...
def contrastive_loss_wo_cross_network(h1, h2):
    f = lambda x: torch.exp(x)
    intra_sim = f(sim(h1, h1))
    inter_sim = f(sim(h1, h2))
    return -torch.log(
        inter_sim.diag() / (intra_sim.sum(dim=-1) + inter_sim.sum(dim=-1) - intra_sim.diag())
    )


def contrastive_loss_wo_cross_view(h1, z):
    f = lambda x: torch.exp(x)
...
class MERIT(nn.Module):
    ...
    def forward(self, aug_adj_1, aug_adj_2, aug_feat_1, aug_feat_2, sparse):
        ...
        l1 = self.beta * contrastive_loss_wo_cross_network(online_pred_one, online_pred_two) + \
            (1.0 - self.beta) * contrastive_loss_wo_cross_view(online_pred_one, target_proj_two.detach())
        
        l2 = self.beta * contrastive_loss_wo_cross_network(online_pred_two, online_pred_one) + \
            (1.0 - self.beta) * contrastive_loss_wo_cross_view(online_pred_two, target_proj_one.detach())
