...
def main():
    ...
    drop_percent = config.drop_ratio 
    ...
    ------------------------------------------------------------
    '''
    print("Begin Aug:[{}]".format(config.aug_type))
    if config.aug_type == 'edge':
    ...
    elif config.aug_type == 'node':
        
        aug_features1, aug_adj1 = aug.aug_drop_node(feat, adj, drop_percent=drop_percent)
        aug_features2, aug_adj2 = aug.aug_drop_node(feat, adj, drop_percent=drop_percent)
        
    elif config.aug_type == 'subgraph':
        
        aug_features1, aug_adj1 = aug.aug_subgraph(feat, adj, drop_percent=drop_percent)
        aug_features2, aug_adj2 = aug.aug_subgraph(feat, adj, drop_percent=drop_percent)

    elif config.aug_type == 'mask':

        aug_features1 = aug.aug_random_mask(feat,  drop_percent=drop_percent)
        aug_features2 = aug.aug_random_mask(feat,  drop_percent=drop_percent)
    ...
    for epoch in range(nb_epochs):
        ...
        if loss < best:
            ...
            best_t = epoch
            cnt_wait = 0
            torch.save(model.state_dict(), './cite_best_dgi.pkl')
        else:
            cnt_wait += 1
    ...
    model.load_state_dict(torch.load('./cite_best_dgi.pkl'))
